...
- 모든 Vue.js템플린슨 스펙을 호환하는 브라우저 및 HTML파서로 구문 분석 할 수 있는 유효한 HTML이다.
...
- boolean 속서을 사용할 때 단순히 true인 경우 v-bind는 조금 다르게 작동한다.

```
<button v-binde:disabled="isButtonDisabled">Button</button>
```
...
- Mustache, v-text, v-html, v-bind, b-show 등이 있다.
...
- 디렉티브의 역할은 표현식의 값이 변경될 때 사이드이펙트를 반응적으로 DOM에 적용한다.
...
- form의 value 값을 가지고 있는 경우에 사용한다.
  ...
  - input, select, textarea 등의 component
